/**
 * Cloudflare Pages Function - OpenAI 투자 컨설팅 보고서
 * POST /api/report
 *
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const prompt = buildPrompt({ age, amount, goal, investorType, portfolio, stocks });
    ...
    const openaiRes = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `당신은 20년 경력의 한국인 투자 컨설턴트입니다.
개인 투자자에게 맞춤형 투자 컨설팅 보고서를 작성합니다.
쉽고 친근한 한국어로 작성하되, 전문적인 분석을 포함합니다.
마크다운 형식으로 작성합니다.
...
function buildPrompt({ age, amount, goal, investorType, portfolio, stocks }) {
  ...
  const portfolioText = portfolio
    ? portfolio.map(p => `- ${p.name}: ${p.pct}% (${p.desc})`).join('\n')
    : '없음';

  const stockText = stocks
    ? stocks.map(s => `- ${s.name}: ${s.reason}`).join('\n')
    : '없음';

  return `
다음 투자자의 프로필을 분석하고 맞춤 투자 컨설팅 보고서를 작성해주세요.

## 투자자 프로필
- 연령대: ${ageMap[age] || age}
- 투자 가능 금액: ${amountMap[amount] || amount}
- 투자 목표: ${goalMap[goal] || goal}
- 투자 유형: ${investorType}
